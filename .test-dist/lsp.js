 ...
 * Supports: TypeScript/JavaScript, Vue, Svelte, Dart/Flutter, Python, Go, Kotlin, Swift, Rust, C/C++
 *
 * Usage:
 *   pi --extension ./lsp.ts
 *
...
export default function (pi) {
    ...
    let hookMode = DEFAULT_HOOK_MODE;
    let hookScope = "global";
    let activity = "idle";
    let diagnosticsAbort = null;
    ...
    function restoreHookState(ctx) {
        ...
        if (entry?.scope === "session") {
            ...
            const legacyEnabled = entry.hookEnabled;
            if (typeof legacyEnabled === "boolean") {
    ...
    function persistHookEntry(entry) {
        pi.appendEntry(LSP_CONFIG_ENTRY, entry);
    }
    ...
    function messageContentToText(content) {
        ...
        if (Array.isArray(content)) {
            return content
                .map((item) => (item && typeof item === "object" && "type" in item && item.type === "text")
                ? String(item.text ?? "")
                : "")
                .filter(Boolean)
                .join("\n");
    ...
    function formatDiagnosticsForDisplay(text) {
        return text
    ...
    async function shutdownLspServersForIdle() {
    ...
    function getServerConfig(filePath) {
        const ext = path.extname(filePath);
        return LSP_SERVERS.find((s) => s.extensions.includes(ext));
    }
    function ensureActiveClientForFile(filePath, cwd) {
    ...
    function extractLspFiles(input) {
        const files = [];
    ...
    function buildDiagnosticsOutput(filePath, diagnostics, cwd, includeFileHeader) {
    ...
    async function collectDiagnostics(filePath, ctx, includeWarnings, includeFileHeader, notify = true) {
            ...
            if (diagnostics.length) {
                ...
                if (notify) {
                    if (ctx.hasUI)
                        ...
                        console.error(report.notification);
                }
            }
            else {
                output = `\n${hint}\n`;
    ...
    pi.registerCommand("lsp", {
        ...
        handler: async (_args, ctx) => {
            ...
            const modeChoice = await ctx.ui.select("LSP auto diagnostics hook mode:", modeOptions.map((option) => option.label));
            if (!modeChoice)
                return;
            ...
            const scopeChoice = await ctx.ui.select("Apply LSP auto diagnostics hook setting to:", scopeOptions.map((option) => option.label));
            if (!scopeChoice)
                return;
    ...
    function agentWasAborted(event) {
        const messages = Array.isArray(event?.messages) ? event.messages : [];
        return messages.some((m) => m &&
            typeof m === "object" &&
            m.role === "assistant" &&
