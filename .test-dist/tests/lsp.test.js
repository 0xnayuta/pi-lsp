...
async function withTempDir(structure, // null = directory, string = file content
fn) {
...
test("LANGUAGE_IDS: C/C++ extensions", async () => {
    assertEquals(LANGUAGE_IDS[".c"], "c", ".c should map to c");
    assertEquals(LANGUAGE_IDS[".h"], "c", ".h should map to c");
    assertEquals(LANGUAGE_IDS[".cpp"], "cpp", ".cpp should map to cpp");
    assertEquals(LANGUAGE_IDS[".cc"], "cpp", ".cc should map to cpp");
    assertEquals(LANGUAGE_IDS[".cxx"], "cpp", ".cxx should map to cpp");
    assertEquals(LANGUAGE_IDS[".hpp"], "cpp", ".hpp should map to cpp");
    assertEquals(LANGUAGE_IDS[".hxx"], "cpp", ".hxx should map to cpp");
});
// ============================================================================
...
test("LSP_SERVERS: has clangd server", async () => {
    const server = LSP_SERVERS.find(s => s.id === "clangd");
    assert(server !== undefined, "Should have clangd server");
    assertIncludes(server.extensions, ".c", "Should handle .c");
    assertIncludes(server.extensions, ".cpp", "Should handle .cpp");
    assertIncludes(server.extensions, ".h", "Should handle .h");
    assertIncludes(server.extensions, ".hpp", "Should handle .hpp");
});
// ============================================================================
...
// ============================================================================
// C/C++ (clangd) root detection tests
// ============================================================================
test("clangd: finds root with CMakeLists.txt", async () => {
    await withTempDir({
        "CMakeLists.txt": "cmake_minimum_required(VERSION 3.10)\nproject(myapp)",
        "src/main.cpp": "int main() { return 0; }",
    }, async (dir) => {
        const server = LSP_SERVERS.find(s => s.id === "clangd");
        const root = server.findRoot(join(dir, "src/main.cpp"), dir);
        assertEquals(root, dir, "Should find root at CMakeLists.txt location");
    });
});
test("clangd: finds root with compile_commands.json", async () => {
    await withTempDir({
        ...
        const server = LSP_SERVERS.find(s => s.id === "clangd");
        const root = server.findRoot(join(dir, "src/main.cpp"), dir);
        assertEquals(root, dir, "Should find root at compile_commands.json location");
    });
});
test("clangd: finds root with Makefile", async () => {
    await withTempDir({
        "Makefile": "all:\n\tg++ -o main main.cpp",
        "main.cpp": "int main() { return 0; }",
    }, async (dir) => {
        const server = LSP_SERVERS.find(s => s.id === "clangd");
        const root = server.findRoot(join(dir, "main.cpp"), dir);
        assertEquals(root, dir, "Should find root at Makefile location");
    });
});
test("clangd: finds root with .git", async () => {
    await withTempDir({
        ".git": null,
        "src/main.cpp": "int main() { return 0; }",
    }, async (dir) => {
        const server = LSP_SERVERS.find(s => s.id === "clangd");
        const root = server.findRoot(join(dir, "src/main.cpp"), dir);
        assertEquals(root, dir, "Should find root at .git location");
    });
});
test("clangd: nested CMake project finds nearest CMakeLists.txt", async () => {
    await withTempDir({
        "CMakeLists.txt": "cmake_minimum_required(VERSION 3.10)\nproject(root)",
        "libs/mylib/CMakeLists.txt": "add_library(mylib STATIC mylib.cpp)",
        "libs/mylib/mylib.cpp": "void foo() {}",
        "libs/mylib/include/mylib.h": "#pragma once",
    }, async (dir) => {
        const server = LSP_SERVERS.find(s => s.id === "clangd");
        const root = server.findRoot(join(dir, "libs/mylib/mylib.cpp"), dir);
        assertEquals(root, join(dir, "libs/mylib"), "Should find nearest CMakeLists.txt");
    });
});
test("clangd: prefers compile_commands.json over CMakeLists.txt", async () => {
    await withTempDir({
        "compile_commands.json": "[]",
        "CMakeLists.txt": "cmake_minimum_required(VERSION 3.10)\nproject(myapp)",
        "src/main.cpp": "int main() { return 0; }",
        ...
        const server = LSP_SERVERS.find(s => s.id === "clangd");
        const root = server.findRoot(join(dir, "src/main.cpp"), dir);
        assertEquals(root, dir, "Should prefer compile_commands.json");
    });
});
test("clangd: returns undefined when no marker files", async () => {
    await withTempDir({
        "main.cpp": "int main() { return 0; }",
    }, async (dir) => {
        const server = LSP_SERVERS.find(s => s.id === "clangd");
        const root = server.findRoot(join(dir, "main.cpp"), dir);
        assertEquals(root, undefined, "Should return undefined when no C++ project markers");
    });
});
// ============================================================================
// Run tests
