...
test("typescript: detects type errors", async () => {
    ...
    try {
        ...
        assert(diagnostics.some(d => d.message.toLowerCase().includes("type") || d.severity === 1), `Expected type error, got: ${diagnostics.map(d => d.message).join(", ")}`);
    }
    finally {
...
test("cpp: detects type errors", async () => {
    ...
    const manager = new LSPManager(dir);
    try {
        await writeFile(join(dir, "CMakeLists.txt"), "cmake_minimum_required(VERSION 3.10)\nproject(test)\nset(CMAKE_CXX_STANDARD 17)\nadd_executable(main main.cpp)");
...
`);
        ...
        if (!edit)
            throw new Error("Expected rename to return WorkspaceEdit");
        assert(edit.changes !== undefined || edit.documentChanges !== undefined, "Expected changes or documentChanges in WorkspaceEdit");
